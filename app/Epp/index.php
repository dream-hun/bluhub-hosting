...
function get_epp_connection() {
    $certpath = 'test.pem';
    $params = array("Username" => "bluhub_rwf", "Password" => "CVQTsh8R64CCdt6G", "Server" => "registry.ricta.org.rw", "Port" => "700", "Certificate" => $certpath, "SSL" => "on");
    try {
...
function search($domain_text, $extension) {
    ...
    $additinal_tlds = array('.co.rw', '.org.rw');
    ...
    if ($response_code == '1000') {
        ...
        for ($i = 0; $i < $number_of_domains; $i++) {
            ...
            array_push($domain_statuses, array('domain_name' => $domain, 'is_available' => $is_available, 'is_the_needed' => $is_the_needed, 'selling_cost' => 'Not yet implemented'));
        }
    } else {
